...
const App: React.FC = () => {
    ...
    const handleTogglePayment = (user: User) => {
        ...
        triggerConfirm(
            isApproving ? "Liberar Acesso" : "Bloquear Acesso",
            isApproving ? `Confirmar pagamento e liberar portal para ${user.name}?` : `Bloquear visualização de ${user.name}?`,
            async () => {
                try {
                    await db.updateUser({ ...user, paymentStatus: isApproving ? PaymentStatus.CONFIRMED : PaymentStatus.AWAITING });
    ...
    return (
        <div className="min-h-screen bg-brand-dark text-gray-100 flex flex-col font-sans">
                    ...
                    <div className="animate-in fade-in duration-1000">
                            ...
                            <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                                <div className="space-y-8 text-center lg:text-left">
                                    <div className="inline-flex items-center gap-3 px-5 py-2 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-[0.3em] text-brand-gold">
                                        <Star size={14} className="animate-pulse" /> Membro da Associação
                                    </div>
                                    <h1 className="text-5xl md:text-8xl font-black text-white uppercase tracking-tighter leading-[0.9]">
                                        Sua Marca na <span className="text-transparent bg-clip-text gold-gradient">Voz VIP</span>
                                    </h1>
                                    ...
                                    <div className="flex flex-wrap gap-6 pt-4 justify-center lg:justify-start">
                                        ...
                                        <button onClick={() => document.getElementById('ads')?.scrollIntoView({behavior:'smooth'})} className="h-20 px-12 bg-white/5 border border-white/10 text-white rounded-3xl font-black uppercase tracking-widest text-sm hover:bg-white/10 transition-all">Explorar</button>
                                    </div>
                                </div>
                        ...
                        <section className="py-32">
                            ...
                            <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-10">
                                {plans.map((p, i) => (
                                    <div key={p.id} className={`glass-panel p-16 rounded-[80px] flex flex-col items-center border-white/5 shadow-2xl transition-all ${i === 1 ? 'scale-105 border-brand-primary/50' : ''}`}>
                                        ...
                                        <ul className="space-y-4 mb-12 text-sm text-gray-500 font-medium">
                                            <li className="flex items-center gap-3"><Check size={18} className="text-brand-gold"/> {p.durationDays} dias ativos</li>
                                            <li className="flex items-center gap-3"><Check size={18} className="text-brand-gold"/> Locução IA inclusa</li>
                                        </ul>
                                        <Button className="w-full h-16 uppercase font-black" onClick={() => setCurrentView('REGISTER')}>Assinar Agora</Button>
                                    </div>
                                ))}
                            </div>
                        </section>

                        <section id="ads" className="max-w-7xl mx-auto px-6 py-32 bg-brand-panel/20 rounded-[100px] mb-32 border border-white/5">
                ...
                {(currentView === 'LOGIN' || currentView === 'REGISTER') && (
                    <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-b from-brand-dark to-brand-panel pt-32">
                        <div className="glass-panel p-20 rounded-[100px] w-full max-w-xl text-center">
                            <h2 className="text-5xl font-black text-white uppercase mb-8 tracking-tighter">{currentView === 'LOGIN' ? 'Entrar VIP' : 'Cadastro VIP'}</h2>
                            <form onSubmit={async (e) => {
                                ...
                                try {
                                    if (currentView === 'LOGIN') {
                                        ...
                                        if (u) {
                                            ...
                                            refreshData();
                                        } else showToast("Acesso inválido.", "error");
                                    } else {
                                        const newUser = { 
                                ...
                                {currentView === 'REGISTER' && (
                                    <>
                                        <input name="name" required placeholder="NOME/EMPRESA" className="w-full bg-white/5 border border-white/10 p-7 rounded-[35px] text-white outline-none" />
                                        <input name="phone" required placeholder="WHATSAPP" className="w-full bg-white/5 border border-white/10 p-7 rounded-[35px] text-white outline-none" />
                                        <div className="grid grid-cols-2 gap-4">
                                            <select name="profession" required className="bg-brand-dark border border-white/10 p-7 rounded-[35px] text-white font-black uppercase text-[10px]">
                                                {categories.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                                            </select>
                                            <select name="planId" required className="bg-brand-dark border border-white/10 p-7 rounded-[35px] text-white font-black uppercase text-[10px]">
                                ...
                                <button type="button" onClick={() => setCurrentView(currentView === 'LOGIN' ? 'REGISTER' : 'LOGIN')} className="text-[10px] text-gray-500 font-black uppercase hover:text-white transition-colors">{currentView === 'LOGIN' ? 'Criar Conta' : 'Já sou Membro'}</button>
                            </form>
                        </div>
                    </div>
                )}
                    ...
                    <div className="flex flex-col md:flex-row min-h-screen pt-32 bg-brand-dark">
                        <aside className="w-full md:w-80 border-r border-white/5 p-10 space-y-4">
                            {[
                                ...
                                <button key={item.id} onClick={() => setAdminSubView(item.id)} className={`w-full flex items-center gap-6 p-6 rounded-[30px] transition-all ${adminSubView === item.id ? 'bg-brand-primary text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
                        ...
                        <main className="flex-1 p-16">
                            {adminSubView === 'CLIENTES' && (
                                <div className="space-y-10">
                                    <h2 className="text-4xl font-black uppercase tracking-tighter text-white">Assinantes</h2>
                                                    ...
                                                    <tr key={u.id} className="hover:bg-white/2 transition-colors">
                                                        ...
                                                        <td className="p-8">
                                                            <div className="flex gap-2">
                                                                ...
                                                                <button onClick={() => triggerConfirm("Deletar Assinante", "Remover conta?", async () => { await db.deleteUser(u.id); refreshData(); }, 'danger')} className="p-4 bg-white/5 text-gray-500 rounded-xl hover:bg-red-600 hover:text-white"><Trash2 size={16}/></button>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                ))}
                            ...
                            {adminSubView === 'PLANOS' && (
                                <div className="space-y-10">
                                    <div className="flex justify-between items-center">
                                        <h2 className="text-4xl font-black uppercase tracking-tighter text-white">Planos Assinatura</h2>
                                        ...
                                        {plans.map(p => (
                                            <div key={p.id} className="glass-panel p-10 rounded-[40px] border-white/5 relative group">
                                                <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                                    <button onClick={() => setEditingPlan(p)} className="p-3 bg-brand-primary rounded-xl text-white"><Edit size={16}/></button>
                                                    <button onClick={() => triggerConfirm("Remover Plano", "Deseja excluir este plano?", async () => { await db.deletePlan(p.id); refreshData(); }, 'danger')} className="p-3 bg-red-600 rounded-xl text-white"><Trash2 size={16}/></button>
                                                </div>
                                        ...
                                        {categories.map(c => (
                                            <div key={c.id} className="p-8 flex items-center justify-between group hover:bg-white/2 transition-all">
                                                ...
                                                <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-all">
                                                    ...
                                                    <button onClick={() => triggerConfirm("Remover Ramo", "Excluir esta categoria?", async () => { await db.deleteCategory(c.id); refreshData(); }, 'danger')} className="p-3 bg-white/5 text-gray-400 hover:text-white rounded-xl hover:bg-red-600 transition-all"><Trash2 size={16}/></button>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            {adminSubView === 'INICIO' && (
                                <div className="space-y-12">
                                    <h2 className="text-4xl font-black uppercase tracking-tighter text-white">Painel Geral</h2>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="glass-panel p-10 rounded-[40px] border-white/5 text-center">
                                            <p className="text-[10px] font-black uppercase text-gray-500">Membros</p>
                                            <h4 className="text-6xl font-black mt-2 text-white">{allUsers.length}</h4>
                                        </div>
                                        <div className="glass-panel p-10 rounded-[40px] border-white/5 text-center">
                                            <p className="text-[10px] font-black uppercase text-gray-500">Anúncios</p>
                                            <h4 className="text-6xl font-black mt-2 text-white">{posts.length}</h4>
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            {adminSubView === 'AJUSTES' && (
                                <form onSubmit={handleSaveConfig} className="space-y-12 max-w-4xl">
                                    <h2 className="text-4xl font-black uppercase tracking-tighter text-white">Ajustes Portal</h2>
                                    <div className="glass-panel p-12 rounded-[50px] space-y-10 border-white/10 shadow-2xl">
                                        ...
                                        <div className="space-y-3">
                                            <label className="text-[10px] font-black uppercase text-gray-500">Imagem Destaque</label>
                                            <div className="aspect-video bg-black/40 rounded-[40px] overflow-hidden relative group border border-white/10">
                                                ...
                                                <button type="button" onClick={() => document.getElementById('heroUp')?.click()} className="absolute inset-0 bg-brand-primary/80 opacity-0 group-hover:opacity-100 flex items-center justify-center font-black uppercase tracking-widest text-xs transition-all text-white">Trocar Foto</button>
                   ...
                   <div className="pt-48 pb-56 max-w-7xl mx-auto px-6">
                        <div className="flex flex-col md:flex-row justify-between items-center gap-12 mb-24 border-b border-white/5 pb-20">
                            <div className="flex items-center gap-10">
                                <div className="w-32 h-32 bg-gradient-to-br from-brand-primary to-purple-800 rounded-[45px] flex items-center justify-center text-white font-black text-5xl">{currentUser.name[0]}</div>
                        ...
                        <div className="glass-panel rounded-[80px] p-16 border-brand-primary/20 mb-32 shadow-3xl relative overflow-hidden group">
                            <div className="absolute top-0 right-0 p-12 opacity-5 pointer-events-none group-hover:scale-110 transition-transform"><Mic size={240} className="text-brand-primary"/></div>
                            <div className="relative z-10 space-y-10">
                                <h2 className="text-3xl font-black uppercase tracking-tighter text-white">Estúdio IA Hélio Júnior</h2>
                                <div className="flex flex-col md:flex-row gap-6">
                                    <input value={magicPrompt} onChange={e => setMagicPrompt(e.target.value)} placeholder="O que deseja divulgar hoje?" className="flex-1 bg-white/5 border border-white/10 p-8 rounded-[35px] text-white outline-none focus:border-brand-primary text-lg" />
                            ...
                            {posts.filter(p => p.authorId === currentUser.id).map(p => (
                                <div key={p.id} className="relative group">
                                    <div className="absolute top-6 right-6 z-20 flex gap-3 opacity-0 group-hover:opacity-100 transition-all">
                                        ...
                                        <button onClick={() => triggerConfirm("Excluir", "Deseja apagar este anúncio?", async () => { await db.deletePost(p.id); refreshData(); }, 'danger')} className="p-4 bg-red-600 rounded-2xl text-white shadow-xl"><Trash2 size={16}/></button>
                                    </div>
                                    <PostCard post={p} />
                                </div>
                            ))}
            ...
            {editingPlan && (
                <div className="fixed inset-0 z-[300] bg-black/95 flex items-center justify-center p-6 backdrop-blur-xl animate-in zoom-in duration-300">
                    <form onSubmit={async (e) => {
                        e.preventDefault();
                        setIsSaving(true);
                        try { await db.savePlan(editingPlan); await refreshData(); setEditingPlan(null); showToast("Plano salvo!"); } catch { showToast("Erro", "error"); }
                        finally { setIsSaving(false); }
                    }} className="glass-panel p-16 rounded-[80px] w-full max-w-xl space-y-10 border-white/10 shadow-3xl text-center">
                        <h3 className="text-3xl font-black uppercase tracking-tighter text-white">Definir Plano VIP</h3>
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <input type="number" step="0.01" value={editingPlan.price} onChange={e => setEditingPlan({...editingPlan, price: Number(e.target.value)})} placeholder="VALOR R$" className="w-full bg-white/5 border border-white/10 p-6 rounded-3xl text-white font-bold" required />
                            ...
                            <textarea value={editingPlan.description} onChange={e => setEditingPlan({...editingPlan, description: e.target.value})} placeholder="BENEFÍCIOS" className="w-full bg-white/5 border border-white/10 p-6 rounded-3xl text-white h-32 resize-none" />
                        </div>
                        <div className="flex gap-4"><Button type="button" variant="outline" onClick={() => setEditingPlan(null)} className="flex-1">Voltar</Button><Button type="submit" className="flex-1" isLoading={isSaving}>Confirmar</Button></div>
                    </form>
                </div>
            )}
            ...
            {editingCategory && (
                <div className="fixed inset-0 z-[300] bg-black/95 flex items-center justify-center p-6 backdrop-blur-xl animate-in zoom-in duration-300">
                    <form onSubmit={async (e) => {
                        e.preventDefault();
                        setIsSaving(true);
                        try { await db.saveCategory(editingCategory); await refreshData(); setEditingCategory(null); showToast("Ramo salvo!"); } catch { showToast("Erro", "error"); }
                        finally { setIsSaving(false); }
                    }} className="glass-panel p-16 rounded-[80px] w-full max-w-md space-y-10 border-white/10 text-center">
                        <h3 className="text-3xl font-black uppercase text-white">Segmento Profissional</h3>
                        ...
                        <Button type="submit" className="w-full h-20 uppercase font-black" isLoading={isSaving}>Finalizar</Button>
                    </form>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[200] bg-black/98 backdrop-blur-3xl flex items-center justify-center p-6 animate-in fade-in duration-300">
                    <form onSubmit={handleSavePost} className="glass-panel p-16 rounded-[100px] w-full max-w-4xl space-y-12 border-white/10 shadow-3xl">
                        <div className="flex justify-between items-center border-b border-white/5 pb-10">
                            <h4 className="text-4xl font-black uppercase text-white tracking-tighter">Editor de Anúncio</h4>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            <div className="space-y-8">
                                ...
                                <textarea value={editingPost.content} onChange={e => setEditingPost({...editingPost, content: e.target.value})} placeholder="O QUE VAI APARECER NO PORTAL..." className="w-full bg-white/5 border border-white/10 p-7 rounded-[35px] text-white h-48 resize-none outline-none leading-relaxed" required />
                            </div>
                            <div className="space-y-8">
                                <div className="aspect-video bg-black/40 rounded-[50px] overflow-hidden border border-white/10 relative group">
                                    ...
                                    <button type="button" onClick={() => document.getElementById('adFile')?.click()} className="absolute inset-0 bg-brand-primary/80 opacity-0 group-hover:opacity-100 flex items-center justify-center font-black uppercase text-white transition-all backdrop-blur-sm">Subir Foto</button>
            ...
            {confirmModal.isOpen && (
                <div className="fixed inset-0 z-[500] bg-black/95 backdrop-blur-md flex items-center justify-center p-6 animate-in zoom-in duration-200">
                    <div className="glass-panel p-16 rounded-[80px] w-full max-w-lg border-white/10 shadow-3xl text-center space-y-8">
                        ...
                        <p className="text-gray-400 font-medium">{confirmModal.message}</p>
            ...
            <footer className="bg-brand-dark py-32 text-center border-t border-white/5">
                <div className="max-w-7xl mx-auto px-6 flex flex-col items-center space-y-12 opacity-50">
                    <div className="flex items-center gap-6">
                      <Radio size={40} />
                      <span className="text-3xl font-black uppercase tracking-tighter">{siteConfig.heroLabel}</span>
                    </div>
                    <p className="text-[10px] font-black uppercase tracking-[0.8em]">Credibilidade em cada palavra</p>
                    <p className="text-[8px] uppercase font-black tracking-widest mt-10">© 2025 {siteConfig.heroLabel} - Portal de Anunciantes VIP</p>
                </div>
            </footer>
        </div>
    );
